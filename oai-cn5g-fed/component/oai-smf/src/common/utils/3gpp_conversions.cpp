...
void xgpp_conv::sm_context_create_from_openapi(
  ...
  if (context_data.guamiIsSet()) {
    ...
    if (context_data.AmfdidIsSet()) {
      // Logger::smf_app().debug("GUAMI %s", context_data.getGuami().c_str());
      std::string amfdid = context_data.getAmfdid();
      CURL *curl;
      CURLcode res;
      curl = curl_easy_init();
      if(curl) {
          Logger::smf_app().debug("CURL time 0\n");
          curl_easy_setopt(curl, CURLOPT_CUSTOMREQUEST, "POST");

          //发送did和mesage给bc的场景二算法
          // curl_easy_setopt(curl, CURLOPT_URL, "http://10.129.182.231:8001/dper/AMFsend/10.129.182.231/8002");
          curl_easy_setopt(curl, CURLOPT_URL, "http://192.168.1.3:8007/dper/setdid");
          ...
          curl_mime_data(part, amfdid.c_str(), CURL_ZERO_TERMINATED);
          ...
          std::string random_number = std::to_string(dis(gen));

        curl_mime_data(part, random_number.c_str(), CURL_ZERO_TERMINATED);

          curl_easy_setopt(curl, CURLOPT_MIMEPOST, mime);
          res = curl_easy_perform(curl);
          curl_mime_free(mime);
          Logger::smf_app().debug("CURL time\n");

          // 根据请求结果进行打印信息
          if (res == CURLE_OK) {
              // 请求成功
             Logger::smf_app().debug("CURL request successful\n");
          } else {
              // 请求失败
              Logger::smf_app().warn("CURL request failed: %s\n", curl_easy_strerror(res));
          }
      }
