...
static void generatePduSessionEstablishRequest(nr_ue_nas_t *nas, as_nas_info_t *initialNasMsg, nas_pdu_session_req_t *pdu_req)
{
  ...
  // 创建套接字
   ...
   // 构造HTTP请求消息
    char *body = nas->uicc->imsiStr;
    char jsonBody[40];
    sprintf(jsonBody, "{\"imsi\":\"%s\"}", body);
    ...
    // 得到当前的秒数
    time_t now0 = tv0.tv_sec;
    
    // 将当前秒数转换为可读的时间格式
    struct tm *timeinfo0;
    timeinfo0 = localtime(&now0);

    // 打印当前时间  
    printf("发送did时间为: %02d:%02d:%02d.%03ld\n", timeinfo0->tm_hour, timeinfo0->tm_min, timeinfo0->tm_sec, tv0.tv_usec / 1000);
    ...
    // 打印当前时间
    printf("接受did时间为: %02d:%02d:%02d.%03ld\n", timeinfo1->tm_hour, timeinfo1->tm_min, timeinfo1->tm_sec, tv1.tv_usec / 1000);
    // 关闭套接字
    close(sock);


    // 定位 ID 字段的起始位置
    const char *id_start = strstr(response, "\"Id\":\"");
    if (id_start != NULL) {
    id_start += 6;  // 跳过 "\"Id\":\"" 的字符
    ...
    if (id_end != NULL) {
        ...
        // 提取 ID 字符串到 DID 数组
        memset(mm_msg->uplink_nas_transport.DID1.did_hello, 0, sizeof(mm_msg->uplink_nas_transport.DID1.did_hello));
        strncpy(mm_msg->uplink_nas_transport.DID1.did_hello, id_start, id_length);
        mm_msg->uplink_nas_transport.DID1.did_hello[id_length] = '\0';  // 确保字符串以 null 终止
        size += strlen(mm_msg->uplink_nas_transport.DID1.did_hello);
    }
    }
